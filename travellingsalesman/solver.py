...
class Solver:
    ...
    def select_seed_node(self, current_solution: list, itr : int) -> int:
        # Select a random node from the current solution as seed for k-opt based on iteration and tabu list
    ...
    def select_candidate(self, current_solution: list, point: int, itr: int, level:int) -> int:
        ...
        candidates = [
            ...
            if node != point and node != curr_child and node != curr_parent and (node not in self.node_mem) and (node not in self.tabu or itr - self.tabu[node] > self.tabu_limit)
        ]
        if not candidates:
            candidate_child = -1
        else:
            ...
            temperature = max(0.1, 10.0 / float(1 + level / node_count**2))
    ...
    def k_optimize(self, current_solution: list, selected_node: int , itr ,level : int) -> list:
        ...
        # reduce the branch factor as level increases expontentially
